...
- `VOX_CUDA_DECODER_INT8=1` enables an experimental INT8 decoder GEMM path (QKV/WO/FFN). This may change outputs slightly; use `./scripts/accuracy_regression.sh` to validate. Default is off even under `VOX_CUDA_FAST=1`. Note: it reserves VRAM for the INT8 weight cache and may reduce the BF16 weight cache auto-size (override with `VOX_CUDA_BF16_CACHE_GIB=<GiB>` if desired).
